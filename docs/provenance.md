...
|---|---|
| `standard/v0.5.0-alpha.4` | **VERIFIED** (2026-09-09) — rebuilt from `d37ca8dcc08527043d9115710573ea2459958908` with Aiken v1.1.23+8949565 in a throwaway clone, upstream's own copy deleted first so it could not be mistaken for the output; sha256 `8136948…`, 164212 bytes, byte-identical. ⚠ Its reproducibility claim is SCOPED: `aiken.toml` pins `aiken-lang/fuzz` to the mutable branch `main`, which cannot reach the blueprint because fuzz is imported only by `.test.ak` modules and test modules are not emitted — that is *why* byte-identity holds despite the mutable pin |
